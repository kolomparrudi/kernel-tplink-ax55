...
static void __setup_per_zone_wmarks(void)
{
	...
	for_each_zone(zone) {
		...
		if ((zone == &zone->zone_pgdat->node_zones[ZONE_NORMAL]) &&
			...
			/* low must be 5MB with following code。 so min:5MB, low:15MB, high:40MB */
			zone->watermark[WMARK_LOW] = min_wmark_pages(zone) * 3;
			zone->watermark[WMARK_HIGH] = min_wmark_pages(zone) * 8;
#else
