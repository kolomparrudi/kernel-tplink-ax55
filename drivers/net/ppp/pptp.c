...
spinlock_t tx_seq_lock;
EXPORT_SYMBOL(tx_seq_lock);
#endif
/* add end  */
...
pptp_gre_seq_offload_callback_t __rcu pptp_gre_offload_xmit_cb;
EXPORT_SYMBOL(pptp_gre_offload_xmit_cb);
#else
...
static int pptp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	/* add by wanghao  */
#if defined (CONFIG_SFE_PPP_HEADER)
	spin_lock_bh(&tx_seq_lock);
	hdr->seq	= htonl(++sync_tx_sequence);
	opt->seq_sent = sync_tx_sequence;
	spin_unlock_bh(&tx_seq_lock);
	/* add end	*/
#else
...
{
	printk("========[%s][%d] enable offload mode for %8X:%8X\n", __FUNCTION__, __LINE__, peer_call_id, peer_ip_addr);
...
{
	printk("========[%s][%d] disable offload mode for %8X:%8X\n", __FUNCTION__, __LINE__, peer_call_id, peer_ip_addr);
